...
**Real app install + offline app shell (added 2026-08-13).** A PWA choice doesn't earn "works with flaky rural internet" for free — it needs a manifest and a service worker actually configured for it, which Phase 1 didn't have until now. Added `vite-plugin-pwa` (Workbox under the hood, the standard, well-audited tool for this rather than a hand-rolled service worker) to precache the app shell (HTML/CSS/JS/icons) at build time, plus a real `manifest.webmanifest` (name, theme colors, a maskable icon) so "Add to Home Screen" gives an actual app icon and standalone window instead of a bookmarked browser tab. Verified by building, serving the output, confirming the service worker installs and precaches, then killing the server entirely and reloading — the full app shell, including sub-routes like `/kid`, still loads with zero network. What this does *not* cover, and can't: live data (balances, invoices, transaction history) still needs a real connection every time, same as before — this only means the app itself opens and is usable (viewing an already-loaded saving target, retrying a payment once signal returns) instead of showing a blank white screen when a connection drops mid-session, which is the actual failure mode this was closing.
