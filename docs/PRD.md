...
A budget ceiling on the NWC connection so the child's spending can never exceed what the parent explicitly allowed, enforced by the parent's own wallet, not by ZapSavr's servers, which never hold the money. A visibility window into savings progress and rough balance, without turning into full surveillance of every 500-sat snack purchase. One-tap revocation that works even if ZapSavr's own backend (if there is one at all) is down, because the permission lives in the NWC connection itself.

**Per-transaction limits, corrected 2026-08-13:** this was originally imagined as a parent-set, wallet-enforced ceiling on any single payment, the same way the weekly budget is. Built instead as something honest given what the architecture actually allows today: there's no channel for a parent's device to push a setting to the kid's device (the same gap that keeps goal visibility parent-side-only, see `docs/ARCHITECTURE.md`), so a *real* per-transaction cap isn't buildable without either a new parent-to-kid sync mechanism or a wallet-level feature NIP-47 doesn't define. What shipped is a kid-set-for-themselves "pause before a big purchase" nudge — an extra confirmation step the kid's own app shows before paying more than a threshold the kid picked. It's a values-aligned feature on its own (a kid choosing their own pause is exactly the low-time-preference practice `README.md` frames this whole app around), just not a parental control. If a real parent-enforced per-transaction cap is wanted later, it needs that sync mechanism designed first, not a client-side setting mislabeled as one.
