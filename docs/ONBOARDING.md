...
**Connect your wallet.** This is the technically hardest step (creating or linking an NWC-capable wallet) and needs the most hand-holding. If the parent already has a compatible wallet, offer a deep link that hands them straight to "approve this connection" inside their existing wallet app. If they don't, offer a guided path to set one up, framed as "Set up your Bitcoin wallet" with a short, reassuring explanation of what it is in normal words, "this is where your money actually lives, ZapSavr just gets permission to use a little of it." Never show a raw NWC connection string, seed phrase, or key on this screen; if the wallet-creation flow surfaces one, treat it as that wallet's business and keep ZapSavr's UI clean of it.

*Which wallet to recommend (checked 2026-08-12):* Alby Hub is still the self-custodial default, but its own setup UI is genuinely rough for a first-time parent, worth saying plainly rather than pretending otherwise. Two real alternatives exist, each with a real trade-off, not a strictly better option:
- **Alby Hub via Alby's managed cloud hosting** (instead of self-hosting) removes the "run a server" step while keeping the connection self-custodial in the way Alby Hub normally works — confirm the exact custody details at signup before recommending this as "just as safe," since managed hosting details can change.
- **Coinos or Primal Wallet** are hosted wallets with NWC built in, connect in a couple of minutes, no node or hub to run at all. The real trade-off: both are custodial, the provider holds the funds, which cuts against the non-custodial point of this whole project (see `README.md`'s "money that isn't permission-based"). Fine as a fast on-ramp for a pilot family testing with small amounts, not a good long-term recommendation once a family is actually saving toward something real.

Whoever builds the wallet-picker screen should present this as a real choice with the trade-off stated in plain language, not silently default to whichever is easiest to code against.
...
## The embedded hot wallet's onboarding is a deliberate exception (2026-08-14)

"Connect your wallet" above says plainly: never show a raw NWC connection string, seed phrase, or key on that screen. The embedded hot wallet (see `docs/ARCHITECTURE.md`'s "Embedded hot wallet") breaks that rule on purpose, and it's worth saying clearly why that's not a contradiction rather than letting it quietly look like one.

It isn't the primary onboarding step. That step, the one every new family goes through, still never shows a key, a string, or a phrase, unchanged. A parent only reaches the hot wallet's setup screen by explicitly choosing "Embedded hot wallet" from the family dashboard, after they already have a PIN set and already understand what ZapSavr is — an advanced, deliberate choice a parent opts into later, not something a first-time parent stumbles into during setup. And unlike an NWC connection string, a relay, or an npub, a seed phrase genuinely can't be hidden behind a plain-language stand-in the way the table near the top of this document hides everything else. It isn't an implementation detail to abstract away, it's the literal, physical recovery mechanism the parent has to write on paper — a euphemism here risks a parent not realizing they actually need to write the words down, which is a real-money mistake, not a jargon one.

What's built follows the spirit of this document even though it breaks the letter of one line: a plain-language warning before anything is shown ("This is real money with no safety net," what recovery does and doesn't cover, stated in normal words), the 12 words themselves with an explicit "write these down" instruction, a two-word confirmation check before the wallet is actually created (so a parent can't skip the backup step by accident), and a "Restore from a backup phrase instead" path for setting the same wallet up again on a new device. See `screens/HotWallet.tsx`. The kid flow is entirely untouched — kids never see or interact with the hot wallet, it's parent-only, the same way the parent's own NWC wallet already was.

Not yet done, and worth flagging against this document's own testing mandate just below: no real parent has been watched going through this flow. The reveal → confirm → restore sequence follows the same conventions this document already treats as proven (see "The pairing moment deserves special care" above for why that matters), but it hasn't been watched with an actual, non-technical parent the way this document says onboarding must be before it's called done. Whoever runs the phase 2 family test should include it.
