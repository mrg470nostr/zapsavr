...
**Closing the loop (2026-08-14):** this line was written when "ZapSavr itself won't hold those keys directly" was still true for everything. That's no longer the whole picture — the embedded hot wallet (`docs/ARCHITECTURE.md`'s "Embedded hot wallet") does hold keys now, opt-in, and it uses a plain BIP39 mnemonic (`@scure/bip39`) rather than the passkey-based derivation flagged here as worth watching. That was a scope choice, not a rejection of passkeys on the merits: passkey/WebAuthn support inside `@breeztech/breez-sdk-spark`'s browser build wasn't investigated in the session that built this, and a mnemonic is the more boring, more portable, more standard option (works the same on any device, restorable by hand, no dependency on a specific browser's WebAuthn implementation actually working inside a PWA). Worth genuinely evaluating passkeys as an alternative before this feature goes past "experimental," since removing the "write down 12 words" step entirely would be a real onboarding win if the SDK's passkey path turns out to work reliably in a PWA context.
